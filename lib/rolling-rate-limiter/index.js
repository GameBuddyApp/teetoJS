  ...
  if (redis) {
    ...
    return function(id, cb) {
      ...
      batch.exec(function(err, resultArr) {
        ...
        var lastValidTime = userSet.length * minDifference + userSet[0];
        var lastAttempt = userSet[userSet.length - 1];
        // check if last attempt was later than last valid and take the correct value
        var nextValid = lastValidTime >= lastAttempt ? lastValidTime + minDifference : lastAttempt + minDifference;

        if (tooManyInInterval) {
          result = Math.max(userSet[userSet.length - maxInInterval] - now + interval);
          result = Math.floor(result / 1000); // convert to miliseconds for user readability.
        } else if(nextValid > now) {
          result = Math.floor((nextValid - now) / 1000);
        } else {
          result = 0;
        }
